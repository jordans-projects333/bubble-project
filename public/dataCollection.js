...
function loadDataIntoItem(dataName, dataPrice, dataImage){
    ...
    await fetch("/length")
        ...
        .then(async data => {
            ...
            switch(dataSettings.filter) {
                case "priority":
                    dataCollection("priority", count, 0)
                    break;
                case "set":
                    dataCollection("set", count, 1)
                    break;
                case "bomb":
                    // if(searchedAlready.bomb.value != data[0].bomb){
                        for(let i = 0; i < data[0].bomb; i++){
                            await dataCollection("bomb", count, 2)
                        }
                        for(let i = 0; i < data[0].set; i++){
                            await dataCollection("set", count, 1)
                        }
                        for(let i = 0; i < data[0].rocks; i++){
                            await dataCollection("rocks", count, 3)
                        }
                        for(let i = 0; i < data[0].bar; i++){
                    ...
                    break;
                case "rocks":
                    dataCollection("rocks", count, 3)
                    break;
                case "barsSalts":
                    dataCollection("bar", count, 4)
                    break;
                }
                count++
